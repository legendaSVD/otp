...
#define FMTF_alt  0x0100
#define FMTF_pad  0x0200
#define FMTF_adj  0x0400
#define FMTF_blk  0x0800
#define FMTF_sgn  0x1000
#define FMTF_cnv  0x2000
#define FMTF_cnV  0x4000
		   ...
		   int width,int precision,int fmt,int* count)
{
    ...
    else if ((fmt & FMTF_pad) && (precision<0)) {
	if (pw)
	    FMT(fn,arg, prefix, pw, *count);
	if (w < precision)
	    ZEROS(fn, arg, precision-w, *count);
