    ...
    if (cgroup_file != NULL) {
        ...
        while (fgets(line_buf, sizeof(line_buf), cgroup_file)) {
            ...
            if (sscanf(line_buf, "%*d:%255[^:]:%4095s\n",
                ...
                ASSERT(version == ERTS_CGROUP_NONE);
                *out = strdup(child_dir);
...
#elif defined(__WIN32__)
#define ERTS_MU_RELATION_PROCESSOR_CORE       0
#define ERTS_MU_RELATION_NUMA_NODE            1
#define ERTS_MU_RELATION_CACHE                2
#define ERTS_MU_RELATION_PROCESSOR_PACKAGE    3
static ETHR_FORCE_INLINE int
rel_cmp_val(int r)
{
    switch (r) {
...
erts_map_win_error_to_errno(DWORD win_error)
{
    ...
    case ERROR_INVALID_BLOCK:			return ENOMEM;
