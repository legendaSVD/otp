                           ...
                           ESockAssocId     assocId)
{
    ...
    SSDBG( descP,
           ("UNIX-ESSIO",
            ...
            __FUNCTION__, descP->sock, sock) );
    poDescP->rBufSz   = descP->rBufSz;
    poDescP->rNum     = descP->rNum;
    poDescP->rNumCnt  = 0;
    poDescP->rCtrlSz  = descP->rCtrlSz;
    poDescP->wCtrlSz  = descP->wCtrlSz;
    poDescP->iow      = descP->iow;
    poDescP->dbg      = descP->dbg;
    poDescP->useReg   = descP->useReg;
...
ERL_NIF_TERM encode_ioctl_flags(ErlNifEnv*       env,
				...
				short            flags)
{
    int          i, flag, num = esock_ioctl_flags_length;
    ERL_NIF_TERM eflags, eflag;
    SocketTArray ta = TARRAY_CREATE(20);
  if (flags == 0) {
    eflags = MKEL(env);
  } else {
                               ...
                               ERL_NIF_TERM*        einfo)
{
    ...
    ERL_NIF_TERM eflags   = MKUI(env, p->snd_flags);
...
void essio_encode_sctp_notif_authkey(ErlNifEnv*                 env,
                                     ...
                                     ERL_NIF_TERM*              eEvent)
{
    ERL_NIF_TERM eflags     = MKUI(env, p->auth_flags);
...
void essio_encode_sctp_notif_sender_dry(ErlNifEnv*                    env,
                                        ...
                                        ERL_NIF_TERM*                 eEvent)
{
    ERL_NIF_TERM eflags = MKUI(env, p->sender_dry_flags);
                                                ...
                                                ERL_NIF_TERM*                   eEvent)
{
    ...
    if (streams != NULL) {
        ...
        if (chunkLen >= 1) {
            unsigned int i;
            SocketTArray sta = TARRAY_CREATE(20);
                   ...
                   int*             errP)
{
    ...
    for (;;) {
        size_t  chunk_size = (size_t) 0x20000000UL;
        size_t  bytes_sent;
        ssize_t res;
        int     error;
