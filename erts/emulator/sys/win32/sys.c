...
static OSVERSIONINFO int_os_version;
...
#define DF_OVR_READY	  1
#define DF_EXIT_THREAD	  2
#define DF_XLAT_CR	  4
#define DF_DROP_IF_INVH   8
#define DF_THREAD_FLUSHED 16
...
static int
get_overlapped_result(AsyncIo* aio,
		      LPDWORD pBytesRead,
		      BOOL wait
		      )
{
    ...
    } else if (aio->pendingError != NO_ERROR) {
	error = aio->pendingError;
	aio->pendingError = NO_ERROR;
	ResetEvent(aio->ov.hEvent);
	DEBUGF(("get_overlapped_result: pending error: %s\n",
		win32_errorstr(error)));
	return error;
    } else if (aio->flags & DF_OVR_READY) {
	aio->flags &= ~DF_OVR_READY;
	*pBytesRead = aio->bytesTransferred;
	ResetEvent(aio->ov.hEvent);
...
{
    HANDLE hToChild = INVALID_HANDLE_VALUE;
    HANDLE hFromChild = INVALID_HANDLE_VALUE;
    ...
    DriverData* dp;
    ErlDrvData retval = ERL_DRV_ERROR_GENERAL;
    int ok;
    int neededSelects = 0;
...
static int application_type (const wchar_t *originalName,
			     ...
			     int *error_return)
{
    ...
    *error_return = ENOENT;
	...
	if(is_quoted) {
	   lstrcpynW(xfullpath, originalName+1, MAX_PATH - 7);
...
ready_output(ErlDrvData drv_data, ErlDrvEvent ready_event)
{
    ...
    while (qsz > 0) {
        ...
        if (!async_write_file(&dp->out, dp->outbuf, dp->outBufSize))
            break;
        dp->out.ov.Offset += dp->outBufSize;
        qsz = driver_deq(dp->port_num, dp->outBufSize);
        dp->outbuf = NULL;
        dp->outBufSize = 0;
    }
