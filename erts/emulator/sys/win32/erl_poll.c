...
#endif
#ifdef DEBUG
#define NoMansLandFill 0xFD
#define DeadLandFill   0xDD
#define CleanLandFill  0xCD
...
typedef struct _Waiter {
    ...
    EventData* evdata[MAXIMUM_WAIT_OBJECTS];
    int active_events;
    int total_events;
    int highwater;
    EventData evdata_heap[MAXIMUM_WAIT_OBJECTS];
    EventData* first_free_evdata;
    HANDLE go_ahead;
    void *xdata;
    erts_tid_t this;
    erts_mtx_t mtx;
...
struct erts_pollset {
    Waiter** waiter;
    int allocated_waiters;
    int num_waiters;
    HANDLE event_io_ready;
    volatile int standby_wait_counter;
    CRITICAL_SECTION standby_crit;
    HANDLE standby_wait_event;
...
static int set_driver_select(ErtsPollSet *ps, HANDLE event, ErtsPollEvents mode)
{
    int i;
    int best_waiter = -1;
    int lowest = MAXIMUM_WAIT_OBJECTS;
    EventData* ev;
    Waiter* w;
#ifdef HARD_POLL_DEBUG
