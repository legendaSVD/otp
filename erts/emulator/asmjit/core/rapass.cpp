...
static ASMJIT_INLINE Error BaseRAPass_calculateInOutKill(
  ...
  for (RABlock* block : pov.iterate()) {
    ...
    BitMutator live_in(block->live_in());
    BitMutator kill(block->kill());
...
ASMJIT_FAVOR_SPEED Error BaseRAPass::build_liveness() noexcept {
  ...
  ArenaVector<uint32_t> n_uses_per_work_reg;
  ArenaVector<uint32_t> n_outs_per_work_reg;
  ArenaVector<uint32_t> n_insts_per_block;
