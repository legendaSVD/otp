...
static inline RegType extend_option_to_reg_type(uint32_t option) noexcept {
  uint32_t pred = (uint32_t(RegType::kGp32) << (0x0 * 4)) |
                  (uint32_t(RegType::kGp32) << (0x1 * 4)) |
                  (uint32_t(RegType::kGp32) << (0x2 * 4)) |
                  (uint32_t(RegType::kGp64) << (0x3 * 4)) |
                  (uint32_t(RegType::kGp32) << (0x4 * 4)) |
                  (uint32_t(RegType::kGp32) << (0x5 * 4)) |
                  (uint32_t(RegType::kGp32) << (0x6 * 4)) |
                  (uint32_t(RegType::kGp64) << (0x7 * 4)) ;
...
Error Assembler::align(AlignMode align_mode, uint32_t alignment) {
  constexpr uint32_t kNopA64 = 0xD503201Fu;
  if (ASMJIT_UNLIKELY(!_code)) {
    return report_error(make_error(Error::kNotInitialized));
  }
