...
struct BaseAddSub {
  uint32_t shifted_op  : 10;
  uint32_t extended_op : 10;
  uint32_t immediate_op: 10;
};
struct BaseAdr {
  M_OPCODE(opcode, 22)
...
struct ISimdVVVe {
  uint32_t regular_op : 26;
  uint32_t regular_vec_type : 6;
  uint32_t element_op : 26;
