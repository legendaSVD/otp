...
static void new_seq_trace_token(Process* p, int);
                                  ...
                                  ErtsTracer meta_tracer)
{
    ...
    if (s->on_load_trace_pattern_flags.breakpoint &&
        ...
        s->on_load_trace_pattern_is_on = !!on;
               ...
               ! flags.breakpoint) {
        if (on) {
            s->on_load_trace_pattern_flags = flags;
            s->on_load_trace_pattern_is_on = !!on;
        }
    } else {
...
new_seq_trace_token(Process* p, int ensure_new_heap)
{
	...
	hp = HAlloc(p, 6);
	SEQ_TRACE_TOKEN(p) = TUPLE5(hp, make_small(0),
				    make_small(0),
				    make_small(0),
				    p->common.id,
