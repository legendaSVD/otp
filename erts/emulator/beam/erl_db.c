...
#include "bif.h"
#define EXI_TYPE     am_type
#define EXI_ID       am_id
#define EXI_ACCESS   am_access
#define EXI_TAB_TYPE am_table_type
#define EXI_BAD_KEY  am_badkey
#define EXI_KEY_POS  am_keypos
#define EXI_POSITION am_position
#define EXI_OWNER    am_owner
#define EXI_NOT_OWNER am_not_owner
#define EXI_ALREADY_EXISTS am_already_exists
#define EXI_DEFAULT  am_default
		...
		Eterm key, Eterm pos_val, Eterm default_obj)
{
    ...
    cret = DB_ERROR_NONE;
    for (iter=list ; is_not_nil(iter); iter = CDR(list_val(iter))) {
	Eterm* pvp = tuple_val(CAR(list_val(iter)));
                  ...
                  Eterm arg2, Eterm arg3, Eterm arg4)
{
	...
	if (arityval(*tpl) == 4) {
	    ...
	    if ((CMP(incr,make_small(0)) < 0) ?
		(CMP(newcnt,threshold) < 0) :
		(CMP(newcnt,threshold) > 0)) {
		newcnt = tpl[4];
	    }
	}
...
retry:
	...
	if (is_boxed(heir_data)) {
	    Eterm* tpv = ((DbTerm*)boxed_val(heir_data))->tpl;
...
void db_info(fmtfn_t to, void *to_arg, bool show)
{
