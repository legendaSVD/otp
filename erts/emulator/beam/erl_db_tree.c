...
static int db_put_dbterm_tree(DbTable* tbl,
                              void* obj,
                              bool key_clash_fail,
...
#define RET_TO_BIF(Term,RetVal) do { 	       	\
	...
	*ret = (Term); 				\
    ...
    if (mpi.key_boundness == MS_KEY_BOUND) {
        ...
        if (this)
            doit_select(&tb->common, this, &sc.common, 0 );
	RET_TO_BIF(sc.accum,DB_ERROR_NONE);
    }
    stack = get_any_stack((DbTable*)tb,stack_container);
    if (reverse) {
...
#define RET_TO_BIF(Term,RetVal) do { 	       	\
	...
	*ret = (Term); 				\
    ...
    if (mpi.key_boundness == MS_KEY_BOUND) {
        ...
        this =  find_node(&tb->common, *sc.common.root, mpi.least, NULL);
        if (this)
            doit_select_count(&tb->common, this, &sc.common, 0 );
	RET_TO_BIF(erts_make_integer(sc.got,p),DB_ERROR_NONE);
    }
    stack = get_any_stack((DbTable*)tb, stack_container);
    if (mpi.key_boundness == MS_KEY_PARTIALLY_BOUND) {
...
#define RET_TO_BIF(Term,RetVal) do { 	       	\
	...
	*ret = (Term); 				\
    ...
    if (mpi.key_boundness == MS_KEY_BOUND) {
        ...
        if (this)
            doit_select_delete(&tbl->common, this, &sc.common, 0
);
	RET_TO_BIF(erts_make_integer(sc.accum,p),DB_ERROR_NONE);
    }
    if (mpi.key_boundness == MS_KEY_PARTIALLY_BOUND) {
...
#define RET_TO_BIF(Term,RetVal) do { 	       	\
	...
	*ret = (Term); 				\
    ...
    if (mpi.key_boundness == MS_KEY_BOUND) {
        ...
        if (pp) {
            doit_select_replace(&tbl->common, pp, &sc.common, 0 );
            reset_static_stack(stack_container);
        }
        RET_TO_BIF(erts_make_integer(sc.replaced,p),DB_ERROR_NONE);
    }
...
static int analyze_pattern(DbTableCommon *tb, Eterm pattern,
                           ExtraMatchValidatorFn *extra_validator,
                           struct mp_info *mpi)
{
    Eterm lst, tpl, ttpl;
