...
typedef struct _ac_node {
    ...
    Sint32 final;
    struct _ac_node *h;
    struct _ac_node *g[ALPHABET_SIZE];
} ACNode;
...
#endif
#define BM_SIZE_SINGLE()     \
(MYALIGN(1) +                       \
 (MYALIGN(sizeof(BMData))))
#define BM_SIZE_MULTI(StrLen) 	            \
((MYALIGN(sizeof(Uint) * (StrLen))) +                      \
 (MYALIGN(sizeof(Uint) * ALPHABET_SIZE)) +                  \
 MYALIGN(StrLen) +                                      \
 (MYALIGN(sizeof(BMData))))
#define AC_SIZE(StrLens)        \
((MYALIGN(sizeof(ACNode)) *                                                   \
((StrLens)+1)) + 	           \
 MYALIGN(sizeof(ACTrie)))
