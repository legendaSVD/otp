                    ...
                    const Eterm *reg)
{
    ...
    if (!(env.mod_nif->flags & ERTS_MOD_NIF_FLG_DELAY_HALT)) {
        result = (*dirty_nif)(&env, codemfa->arity, argv);
    }
    else {
...
int enif_inspect_iolist_as_binary(ErlNifEnv* env, Eterm term, ErlNifBinary* bin)
{
    ...
    if (is_nil(term)) {
	bin->data = (unsigned char*) &bin->data;
	bin->size = 0;
	bin->ref_bin = NULL;
	return 1;
    }
...
{
    Binary* refbin;
    refbin = erts_bin_drv_alloc_fnf(size);
    if (refbin == NULL) {
	return 0;
    }
    bin->size = size;
...
void* enif_alloc_resource(ErlNifResourceType* type, size_t data_sz)
{
    ...
    ASSERT(type->owner && type->next && type->prev);
    resource->type = type;
