...
#endif
extern ErtsStaticDriver driver_tab[];
erts_driver_t *driver_list;
erts_rwmtx_t erts_driver_list_lock;
static erts_tsd_key_t driver_list_lock_status_key;
static erts_tsd_key_t driver_list_last_error_key;
ErtsPTab erts_port erts_align_attribute(ERTS_CACHE_LINE_SIZE);
...
kill_port(Port *pp)
{
    ...
    erts_ptab_delete_element(&erts_port, &pp->common);
...
Port *
erts_open_driver(erts_driver_t* driver,
		 Eterm pid,
		 char* name,
		 SysDriverOpts* opts,
		 int *error_type_ptr,
...
static void deliver_bin_message(Port*  prt,
				Eterm to,
				char* hbuf,
				ErlDrvSizeT hlen,
				ErlDrvBinary* bin,
...
static void dump_port_state(fmtfn_t to, void *arg, erts_aint32_t state)
{
    ...
    rest = state;
    while (rest) {
        erts_aint32_t chk = (rest ^ (rest-1)) & rest;
        char* s;
        rest &= ~chk;
        switch (chk) {
...
static void dump_port_task_flags(fmtfn_t to, void *arg, Port* p)
{
    ...
    while (flags) {
        erts_aint32_t chk = (flags ^ (flags-1)) & flags;
        char* s;
        flags &= ~chk;
        switch (chk) {
...
SysIOVec* driver_peekq(ErlDrvPort ix, int* vlenp)
{
