...
BIF_RETTYPE system_info_1(BIF_ALIST_1)
{
	...
	BIF_RET(AM_tag);
        ...
        xtra = 2*8  + MAP_HEADER_FLATMAP_SZ + 2 ;
                   ...
                   Eterm item)
{
	...
	if (hpp)
	    res = ERTS_PORT_GET_CONNECTED(prt);
	if (szp) {
	    res = am_true;
	    goto done;
	}
...
static void os_info_init(void)
{
    ...
    char* buf = erts_alloc(ERTS_ALC_T_TMP, 1024);
    Eterm* hp;
    Eterm tuple;
