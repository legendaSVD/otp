...
typedef enum {
    ...
    matchPushVResult,
    matchPushExpr,
    matchPushArrayAsList,
    matchPushArrayAsListU,
    matchTrue,
    matchOr,
    matchAnd,
    matchOrElse,
...
{
    (void) get_match_pseudo_process(c_p, 0);
}
...
restart:
	...
	clause_start = DMC_STACK_NUM(text);
	DMC_PUSH(stack,NIL);
	for (;;) {
			       ...
			       bool *constant)
{
    ...
    *constant = false;
    DMC_PUSH(*text, matchSetReturnTrace);
    if (++context->stack_used > context->stack_need)
	context->stack_need = context->stack_used;
    return retOk;
}
                               ...
                               bool *constant)
{
    ...
    *constant = false;
    DMC_PUSH(*text, matchSetExceptionTrace);
    if (++context->stack_used > context->stack_need)
	context->stack_need = context->stack_used;
    return retOk;
}
			       ...
			       bool *constant)
{
    ...
    *constant = false;
    DMC_PUSH(*text, matchProcessDump);
    if (++context->stack_used > context->stack_need)
	context->stack_need = context->stack_used;
    return retOk;
}
