...
#define RETURN_STATE()							\
	...
	case TTBEncode:
		...
		EXPORT_CONTEXT();
		bytes = (byte *) result_bin->orig_bytes;
...
init_done:
	...
	case SMALL_BIG_EXT:
	    ...
	    n = ep[0];
	    SKIP2(n, 1+1);
	    heap_size += 1+(n+sizeof(Eterm)-1)/sizeof(Eterm);
	    break;
	case LARGE_BIG_EXT:
	    CHKSIZE(4);
	    n = get_uint32(ep);
	    ...
	    SKIP2(n,4+1);
	    heap_size += 1+1+(n+sizeof(Eterm)-1)/sizeof(Eterm);
	    break;
	case ATOM_EXT:
	    CHKSIZE(2);
	    n = get_int16(ep);
	...
	default:
        ...
        if (terms == lext_term_end) {
            ErtsBlockHashState lext_state_buf, *lext_state;
            const byte *ep_start = lext_hash + 4 ;
            Sint len = ep - ep_start;
            Uint chunk_len;
            Uint32 hash;
            ASSERT(lext_hash);
