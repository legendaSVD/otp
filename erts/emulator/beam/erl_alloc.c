...
#ifndef SMALL_MEMORY
    ip->init.util.mmbcs 	= 128*1024;
#else
    ip->init.util.mmbcs 	= 32*1024;
#endif
...
#ifndef SMALL_MEMORY
    ip->init.util.mmbcs 	= 2*1024*1024;
#else
    ip->init.util.mmbcs 	= 1*1024*1024;
#endif
...
#ifndef SMALL_MEMORY
    ip->init.util.mmbcs 	= 1024*1024;
#else
    ip->init.util.mmbcs 	= 256*1024;
#endif
...
UWord erts_alc_test(UWord op, UWord a1, UWord a2, UWord a3)
{
	...
	case 0xf17: {
	    ...
	    void* new_creating_mbc = *(void**)a2;
	    void* new_destroying_mbc = *(void**)a3;
	    int i;
	    for (i=0; i < ts->size; i++) {
