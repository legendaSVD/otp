...
struct mp_info {
    bool something_can_match;
    bool key_given;
    struct mp_prefound dlists[10];
...
struct ext_segtab {
    ...
    struct segment** prev_segtab;
    UWord prev_nsegs;
    UWord nsegs;
    struct segment* segtab[1];
};
...
int db_create_hash(Process *p, DbTable *tbl)
{
	...
	if (erts_ets_rwmtx_spin_count >= 0)
	    rwmtx_opt.main_spincount = erts_ets_rwmtx_spin_count;
	tb->locks = (DbTableHashFineLockSlot*) erts_db_alloc(ERTS_ALC_T_DB_SEG,
                                                             (DbTable *) tb,
...
static int match_traverse(traverse_context_t* ctx,
                          Eterm pattern,
                          Sint chunk_size,
                          Sint iterations_left,
                          Eterm* ret)
{
    DbTableHash* tb = ctx->tb;
    Sint slot_ix;
    HashDbTerm** current_ptr;
    HashDbTerm* saved_current;
    struct mp_info mpi;
                                      ...
                                      Eterm match_res)
{
    ...
    if (NFIXED(ctx->base.tb) > ctx->fixated_by_me) {
...
static int analyze_pattern(DbTableHash *tb, Eterm pattern,
                           ExtraMatchValidatorF* extra_validator,
                           struct mp_info *mpi)
{
    Eterm *ptpl;
    Eterm lst, tpl, ttpl;
...
db_finalize_dbterm_hash(int cret, DbUpdateHandle* handle)
{
    ...
    Sint nitems;
    ERTS_LC_ASSERT(IS_HASH_WLOCKED(tb, &lck_ctr->lck));
