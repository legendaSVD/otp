...
erts_init_time_sup(int time_correction, ErtsTimeWarpMode time_warp_mode)
{
    ...
    if (!time_sup.r.o.os_monotonic_time_disable) {
        time_sup.r.o.os_corrected_monotonic_time = 0;
...
send_time_offset_changed_notifications(void *new_offsetp)
{
    ...
    ErtsTimeOffsetMonitorInfo *to_mon_info = NULL;
    Uint no_monitors;
    char *tmp = NULL;
#ifdef ARCH_64
	...
	alloc_sz += 6*sizeof(Eterm);
	alloc_sz += ERTS_MAX_SINT64_HEAP_SIZE*sizeof(Eterm);
