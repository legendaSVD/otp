...
int distribution_info(fmtfn_t to, void *arg)
{
    DistEntry *dep;
                    ...
                    Uint32 connection_id, Eterm type, Eterm reason)
{
    ...
    for (i = 0; i < no; i++) {
        ErtsHeapFactory hfact;
        Eterm tmp_heap[ERTS_MON_NODES_MAX_INFO_SZ__(3)];
        Eterm *hp, msg;
        Uint hsz;
	...
	if (!opts) {
        ...
        if (!opts) {
            ...
            Eterm tup;
            Eterm info;
            if (opts & ERTS_NODES_MON_OPT_INFO_MAP) {
                Uint map_size = 0;
                Eterm ks[3], vs[3];
