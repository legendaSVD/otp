...
static struct ErtsSchedSuspend_ {
    ...
    ErtsProcList *chngq;
    Eterm changer;
    ErtsMultiSchedulingBlock nmsb;
    ErtsMultiSchedulingBlock msb;
...
#define ERTS_SCHED_UTIL_IGNORE_IMBALANCE_DIFF 5000
static ERTS_INLINE Uint64
	...
	while (proc && max_processes_to_steal) {
        ...
        if (first_stolen_proc) {
            ...
            *result_proc = first_stolen_proc;
            ...
            n_procs_stolen[first_stolen_proc_prio]--;
		     ...
		     )
{
    ...
    init_swtreq_alloc();
    init_screq_alloc();
    erts_atomic32_init_nob(&debug_wait_completed_count, 0);
			 ...
			 Eterm st_result, int normal_sched)
{
	...
	hsz = st->req_id_sz + st_result_sz + 4 ;
...
erts_execute_dirty_system_task(Process *c_p)
{
	...
	if (erts_proc_sig_fetch(c_p))
            c_p->flags &= ~F_DIRTY_GC_HIBERNATE;
        else {
...
Eterm
erl_create_process(Process* parent,
		   Eterm mod,
		   Eterm func,
		   Eterm args,
		   ErlSpawnOpts* so)
{
    ...
    ASSERT(rq);
    p = alloc_process(rq, bound, state);
    if (!p) {
...
delete_process(Process* p)
{
	...
	erts_atomic_set_nob(&p->psd, (erts_aint_t) NULL);
