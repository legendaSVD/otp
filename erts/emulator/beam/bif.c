...
BIF_RETTYPE register_2(BIF_ALIST_2)
{
...
BIF_RETTYPE string_list_to_float_1(BIF_ALIST_1)
{
	...
	} else if (IS_DOT(CAR(list_val(list)))) {
	    switch (part) {
	    case INT:
		part = FRAC;
		break;
	...
	} else if (IS_E(CAR(list_val(list)))) {
	    switch (part) {
	    case FRAC:
	...
	} else if (IS_DIGIT(CAR(list_val(list)))) {
	    switch (part) {
	    case SIGN:
		part = INT;
		break;
...
BIF_RETTYPE halt_2(BIF_ALIST_2)
{
    ...
    int flush = 1;
    ErtsMonotonicTime halt_flush_timeout = -1;
