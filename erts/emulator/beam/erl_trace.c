...
{
    int ts_type_flag = ts_type & -ts_type;
...
{
    ErtsSysMonMsgqLow *thiz = erts_alloc(ERTS_ALC_T_HEAP_FRAG,
...
remove_msgq_low_session(Process *p, ErtsSysMonMsgqLow *thiz)
{
    ...
    erts_free(ERTS_ALC_T_HEAP_FRAG, thiz);
}
void
erts_clear_all_msgq_low_sessions(Process *p)
{
    ...
    while (that) {
        next = that->next;
        erts_free(ERTS_ALC_T_HEAP_FRAG, that);
        that = next;
    }
    p->sig_qs.flags &= ~FS_MON_MSGQ_LEN_LOW;
}
...
#define MAP_SIZE 4
        Eterm argv[5], local_heap[3+MAP_SIZE  + (MAP_SIZE+1 )];
                              ...
                              ErtsTraceSession* session)
{
    ...
    ref = erts_alloc(ERTS_ALC_T_HEAP_FRAG,
...
void delete_tracer_ref(ErtsPTabElementCommon* t_p, ErtsTracerRef *ref)
{
    ...
    *prev_p = ref->next;
    erts_free(ERTS_ALC_T_HEAP_FRAG, ref);
