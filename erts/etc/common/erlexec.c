...
static int start_detached = 0;
static const char* emu_type = NULL;
static const char* emu_flavor = DEFAULT_SUFFIX;
#ifdef __WIN32__
static char *start_emulator_program = NULL;
static char* key_val_name = ERLANG_VERSION;
static char* boot_script = NULL;
static char** config_scripts = NULL;
...
#endif
{
    ...
    free_env_val(s);
#endif
    initial_argv_massage(&argc, &argv);
    i = 1;
#ifdef __WIN32__
	...
	if (!process_args) {
		...
		case 'c':
		  ...
		  case 'e':
			...
			ADD_BOOT_CONFIG;
			add_arg(argv[i]);
		    } else if (strcmp(argv[i], "-emu_args") == 0) {
			verbose = 1;
