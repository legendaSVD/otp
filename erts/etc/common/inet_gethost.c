...
#define UNIT_IPV4  4
#define UNIT_IPV6 16
typedef unsigned char AddrByte;
...
#define WORKER_EMPTY   0
#define WORKER_FREE    1
#define WORKER_BUSY    2
#define WORKER_STALLED 3
#define CHILDWAIT_TMO 1
#define DOMAINNAME_MAX 258
typedef struct {
    WorkerState state;
#ifdef WIN32
    DWORD pid;
...
#endif
    if(!greedy_threshold) {
	greedy_threshold = (3*num_workers)/4;
	if (!greedy_threshold) {
				...
				size_t *preply_size)
{
    ...
    int string_need = strlen(errstring) + 1;
    unsigned need;
    AddrByte *ptr;
    need = PACKET_BYTES + 4  + 1  + string_need;
    if (*preply_size < need) {
	if (*preply_size == 0) {
			  ...
			  AddrByte **preply, size_t *preply_size)
{
    ...
    for (num_addresses = 0; he->h_addr_list[num_addresses] != NULL;
	 ++num_addresses)
	;
    strings_need = strlen(he->h_name) + 1;
    num_strings = 1;
    if (he->h_aliases) {
...
BOOL create_mesq(MesQ **q)
{
    ...
    InitializeCriticalSection(&(tmp->crit));
    tmp->shutdown = 0;
    tmp->first = NULL;
    tmp->last = NULL;
    *q = tmp;
